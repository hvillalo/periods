...
En este caso, es conveniente centrar la serie a media cero para facilitar el ajuste. Para ello se utiliza el argumento `center.x = TRUE`. Aunque este es su valor por defecto, lo incluimos para no olvidarlo en los pasos siguientes.
...
Como puede verse en el gráfico, los valores del número de manchas solares tienen media cero debido al ajuste, además no se muestra el tiempo en años porque en la matriz del modelo no se incluyó la tendencia lineal. Sin embargo, la gráfica correcta se puede generar de manera muy sencilla:
